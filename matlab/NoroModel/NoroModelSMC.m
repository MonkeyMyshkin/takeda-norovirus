...
%DATA
load('data/GermanCaseNotification.mat')  %survstat data
load('data/GermanPopulation.mat')        %German population sizes per age
load('data/GermanContactData.mat')       %Contact data from POLYMOD
load('data/GermanParticipantData.mat')   %participant data from POLYMOD
...
parfor particleIndex=1:NumberParticles
    ...
    chistart=2/365;   %never estimate this- duration of complete immune period
    ReportingStart=[unifrnd(0,1),unifrnd(0,0.5),unifrnd(0,0.5),unifrnd(0,0.5),unifrnd(0,0.5), unifrnd(0,1)]; %not full prior in all cases- can adjust
    
    
    ParamCurrent=abs([alphastart, qstart, omega1start, omega2start, nustart, ...
                    deltastart, epsilonstart, sigmastart, psistart, gammastart, chistart, ReportingStart]);
...
for IterIndex=1:50
    ...
    ParamSliceResampledPARTICLE=ResampledPARTICLE(:,1:17);
    DispSliceResampledPARTICLE=ResampledPARTICLE(:,18);
    kSliceResampledPARTICLE=ResampledPARTICLE(:,19);
    dSliceResampledPARTICLE=ResampledPARTICLE(:,20);
    dampSliceResampledPARTICLE=ResampledPARTICLE(:,21:22);
    thetaSliceResampledPARTICLE=ResampledPARTICLE(:,23:end);
    ...
    parfor particleIndex=1:NumberParticles
        ...
        if IterIndex>1 && rand(1)<0.9  %this adapts 90% of the time
            ...
            adapt=cov(PARTICLE(:,[2,3,4,5,6,8,10,12:17,18,19,20,21:22,23:end]))* 2; 
            %indexes correspond to the parameters we are estimating
            
            New=(mvnrnd([ParamCurrent([2,3,4,5,6,8,10,12:17]),DispersionPar,kPar,dPar,dampPar,thetaPar],adapt));
            ...
            New=(mvnrnd([ParamCurrent([2,3,4,5,6,8,10,12:17]),DispersionPar,kPar,dPar,dampPar,thetaPar],proposal));
        ...
        OUTofBOUNDS= min(PRP)<0 || isinf(Priors(ParamProp(1:11),thetaProp,ParamProp(12:17),DispersionProp,...
        ...
        TransitionKernel(particleIndex)=mvnpdf(nP([2,3,4,5,6,8,10,12:17,18,19,20,21:22,23:end]),CurrentResampledPARTICLE([2,3,4,5,6,8,10,12:17,18,19,20,21:22,23:end]),TransitionKernelCovariance);
