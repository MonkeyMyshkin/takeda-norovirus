...
%      omega2, nu, delta, epsilon, sigma, psi, gamma, chi. 
...
options=odeset('NonNegative',1:10*Lmax);%ODE solver option
...
[TIME,X]=ode45(@(t,Y)NoroModelARODE(t,Y,Param(1:11) ,mu,theta(1),CM),tspan,[1e6*fixedPoints,zeros(1,2*Lmax)],options);  %ICSarescaled up to improve accuracy

%time to average of 2 years

[TIMENEW , Xnew ]=ode45(@(t,Y)NoroModelARODE(t,Y,Param(1:11),mu ,theta(1),CM),(TIME(end):1:TIME(end)+2*365),X(end,:),options);

%
InitialConditions = mean(Xnew(:,1:8*Lmax));
