function [ TIME,SimulationResult ] = SimulateSeasons( Param,mu ,theta, ContactMatrix , x0)
%%
...
%SEASON ONE
tspan=1:1:33*7;
[TIME,X]=ode45(@(t,Y)NoroModelARODE(t,Y,Param,mu,theta(1),CM),tspan,[x0,zeros(1,2*Lmax)],options);
...
for seasonIndex= 2 : 8
    ...
    [ TIMEnew , Xnew ]=ode45(@(t,Y)NoroModelARODE(t,Y,Param,mu,theta(seasonIndex),CM),(TIME(end):1:TIME(end)+52*7),X(end,:),options);
...
%SEASON NINE
[ TIMEnew , Xnew ]=ode45(@(t,Y)NoroModelARODE(t,Y,Param,mu,theta(9),CM),(TIME(end):1:TIME(end)+21*7),X(end,:),options);
